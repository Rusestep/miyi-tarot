...
一款完整收录 78 张 Rider–Waite–Smith 塔罗牌的沉浸式在线抽牌应用。支持单张指引与三张牌阵、正逆位解读、完整牌库浏览，以及根据指针或手机姿态实时变化的 3D 倾斜、物理翻牌、动态高光和阴影效果。
...
- 手机端可按需启用陀螺仪体感，并支持横竖屏自动换算和重新校准
- 逆位保留独立解读与标记，牌面图像始终正向显示
...
手机体感只在用户主动授权后启用，方向数据仅用于当前页面的卡牌动画，不会上传或保存。iPhone/iPad 需通过 HTTPS 访问并点击“开启手机体感”后授予权限；Cloudflare Pages 默认提供 HTTPS。
